<template>
  <div class="upload-container">
    ...
    <el-dialog  v-dialogDrag append-to-body :visible.sync="dialogVisible">
      <el-upload class="editor-slide-upload" :action="actionurl" :data="uploadData" :multiple="true" :file-list="fileList" :show-file-list="true" list-type="picture-card" :on-remove="handleRemove" :on-success="handleSuccess" :before-upload="beforeUpload">
    ...
    handleSuccess(response, file, fileList) {
      ...
      for (let i = 0, len = objKeyArr.length; i < len; i++) {
        if (this.listObj[objKeyArr[i]].uid === uid) {
          ...
          this.listObj[objKeyArr[i]].url = process.env.BASE_API + 'images/' + response
