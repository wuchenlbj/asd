...
<template>
  <div class="uploader-container">
    ...
    <div v-loading="loadingToQD" element-loading-text="正在保存文档关联信息入库,请稍后..">
      <!-- 导航部分 -->
      <div class="toptext">
  ...
  methods: {
    ...
    initUpload () {
      ...
      this.uploader.on('uploadFinished', function () {
        ...
        _this.fileQueryList.PCBM = NewGuid()

        if (_this.fileQueryList.XmList.length === 0) {
          // GNM 根据新的数据格式修改
          msg.showwarning('没有获取到项目信息请重试') // GNM
          _this.hidedwin()
          return false
        }
        _this.loadingToQD = true
        ...
        saveFileXMData(_this.fileQueryList)
          .then(response => {
            if (response.data.length > 0) {
              ...
              _this.query.PCBM = _this.fileQueryList.PCBM
              // 如果fid 是根清单 则保存关系
              _this.fileQueryList = {
                // GNM 根据新的数据格式修改
                FilesList: [],
              ...
              _this.loadingToQD = false
              // 3. 重新获取当前文件夹下的文档
              _this.getList(_this.folderfid, -1, '')

              // bus.$emit('on-getXMInfo')// GNM 此处会触发多次请求，改为在前端标记
              ...
              _this.loadingToQD = false
              // 3. 获取数据
              // _this.getList(_this.folderfid, -1, '')// GNM
    ...
    saveXMXX (file, filephpath, index) {
      ...
      // _this.fileQueryList.push(fileQuery)
      _this.fileQueryList.FilesList.push(fileQuery) // GNM 根据新的数据格式修改
    },
    // 上传完毕隐藏处理
    hidedwin () {
      this.isshow = false
