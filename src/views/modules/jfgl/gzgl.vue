...
<template>
  <div class="jfsb">
    <el-row :gutter="10">
      <el-col :span="12">
        <div v-if="xmxxRow&&XMHTXX.length>0" class="pageTiptitle">
          该课题计划费用{{xmxxRow.ZFY}}万元，累积签订合同{{XMHTXX.length}}份，
          合同签订累积额{{xmxxRow.HTJ}}万元。累积挂账{{GZZJE}}万元</div>
        <p class="pageTabletitle">合同列表信息</p>

        <el-table v-loading='loading' :height="tableHeight" @cell-click='cellclickHT' highlight-current-row ref="htqt" border :data="XMHTXX" style="width: 100%;margin-top: 5px;" header-row-class-name="headclass">
          ...
          <el-table-column prop="GZJE" label="累积挂账" width="80"></el-table-column>
          <el-table-column prop="HTTYPEMC" label="合同类型" width="90"></el-table-column>
          ...
          <el-table-column prop="LXR" label="联系人" show-overflow-tooltip width="70"></el-table-column>
      ...
      <el-col :span="12">
        ...
        <div style="margin-left:100px;position:absolute;top:-5px;" >
          <el-button :disabled="!selectHTrow.KTBH" type="primary" size="mini" icon="el-icon-edit" @click="add">添加挂账</el-button>
        </div>
        <p class="pageTabletitle">合同挂账信息
        </p>

        <el-table v-loading="loading2" :height="tableHeight" border :data="GZtableData" @row-dblclick="rowdblclick" header-row-class-name="headclass" style="width: 100%;margin-top: 5px;"  >

          <el-table-column label="挂账金额(元)" prop="GZJE" width="110"></el-table-column>
          <el-table-column label="挂账批次" prop="GZPC" width="110">
            <template slot-scope="scope">
              <span class="span-title">{{scope.row.GZPC|getPC(scope.row.GZPC)}}</span>
            </template>
          </el-table-column>
          <el-table-column label="经办人" prop="JBRNAME" min-width="80"> </el-table-column>
          <el-table-column label="挂账日期" width="130">
            <template slot-scope="scope">
    ...
    <!-- 添加编辑合同 -->
    <el-dialog v-dialogDrag  append-to-body title="挂账信息" width="800px" :visible.sync="dialogFormVisible">
      <table class="tbinfo" v-if="xmxxRow">
        ...
        <tr>
          <td class="t1">合同价</td>
          <td class="t2"> {{this.xmxxRow.HTJ}}万元</td>
          <td class="t1">累积挂账</td>
      ...
      <el-form :model="form" ref="ruleForm" :rules="rules" :inline="true" label-width="90px" style="margin:10px 0;">
        ...
        <el-form-item label="挂账金额" prop="GZJE">
          <el-input v-model="form.GZJE" style="width:250px;" placeholder="挂账金额">
            <template slot="append">万元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="挂账批次" prop="GZPC">
          <el-select filterable v-model="form.GZPC" @change="" style="width: 250px;" placeholder="挂账批次">
        ...
        <el-form-item style="margin-left:90px;">
          <el-button type="primary" :loading="loadinght" @click="submitForm('ruleForm')">添加挂账</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        GZPC: [{ required: true, message: '请输入挂账批次', trigger: 'change' }],
        GZJE: [{ type: 'float', required: true, trigger: 'blur',
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.form.TYPE == 'add') {
            var htj = parseFloat(this.selectHTrow.HTJ)
            var gzlje = parseFloat(this.selectHTrow.GZJE)
            var gz = parseFloat(this.form.GZJE)
            if (htj < (gzlje + gz)) {
              msg.showwarning('挂账金额累积不能超过合同额,请重新输入')
              return false
            }
          }
          ...
          saveGZData(this.form).then(response => {
