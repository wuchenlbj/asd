...
<template>
  <div class="jfsb">
    ...
    <el-table ref="zftj" v-loading='loading' :height="tableHeight" highlight-current-row border :data="XMHTXX" style="width: 100%;" header-row-class-name="headclass">
      ...
      <el-table-column label="累积支付金额(万元)" prop="GZJE"></el-table-column>
      ...
      <el-table-column label="支付记录">
        ...
        <el-table-column label="金额(万元)" prop="DECZFJE" width="100"></el-table-column>

        <el-table-column prop="DSCRQ" label="第三次时间" width="90">
          <template slot-scope="scope">
            <span class="span-title" v-if="scope.row.DSCRQ">{{scope.row.DSCRQ|parseTime3('yyyy-MM')}}</span>
          </template>
        </el-table-column>
        <el-table-column label="金额(万元)" prop="DSCZFJE" width="100"></el-table-column>
      </el-table-column>
...
import { getZFTJData } from "api/qmys/index.js"
...
export default {
  ...
  mounted () {
    this.setHeight()
    this.tbobj = this.$refs.zftj
  },
  methods: {
    setHeight () {
      this.tableHeight = this.panelHeight - 58
    },
    changeyear (val) {
      this.form.JHND = '' + val
      // 查询年度数据
    ...
    _getGZTJData () {
      var _this = this
      this.loading = true
      getZFTJData(_this.form).then(response => {
        this.loading = false
