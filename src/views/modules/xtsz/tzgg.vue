...
<template>
  <div class="contation">
    ...
    <el-table :height="tableHeight" @row-dblclick="rowdblclick"  @row-click="rowclick" highlight-current-row  v-loading="loading" :data="tableData" border style="width: 99.8%" header-row-class-name="headclass">
      ...
      <el-table-column prop="FBDATE" label="发布日期" width="90"></el-table-column>
    ...
    <!-- 表单 -->
     <el-dialog  v-dialogDrag title="信息编辑"  width="900px"  top="2vh"   :visible.sync="dialogFormVisible">
      <el-form  v-loading='loadingform' :inline="true" style="width:800px;margin-left:20px;" ref="ruleForm" :model="form" label-width="90px">
        ...
        <el-form-item label="文件号" prop="SUBTITLE">
            <el-input style="width:290px"  placeholder="文件号" v-model="form.SUBTITLE"></el-input>
        </el-form-item>
        <el-form-item label="选择采油厂"  prop="ZT">
          <el-select  style="width:290px"  v-model="form.ZT" placeholder="选择采油厂">
            <el-option label="" value="等待添加"></el-option>
          </el-select>
        </el-form-item>
        ...
        <div class="editor-container"   prop="NEWSCONTENT">
         ...
         <el-form-item>
            <el-upload  class="upload-demo"
              ...
              <div slot="tip" class="el-upload__tip">选择通知公告的相关进行上传</div>
            </el-upload>
        </el-form-item>
        <el-form-item>
...
import { getTZList, getDetail, saveOneTZGG, dropTzgg, getDocDetail } from 'api/xtsz/sy'
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          saveOneTZGG(this.form).then(response => {
    ...
    del() {
      ...
      this.$confirm("确定要删除标题为《" + this.selectRow.TITLE + "》的信息吗？", "删除提示", {
        ...
        .then(() => {
          ...
          dropTzgg({ id: _this.selectRow.ID, tbname: 'BD_MSG_NOTICE' }).then(response => {
            if (response.data) {
